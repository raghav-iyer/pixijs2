...
export function TaskResultOverlay() {
  ...
  useEffect(() => {
    ...
    const duration = taskResult.type === 'success' ? 2500 : 2000;
        ...
        <div
          style={{
            ...
            background: isSuccess
              ? 'radial-gradient(circle, rgba(34,197,94,0.15) 0%, rgba(0,0,0,0.4) 70%)'
                ...
                {Array.from({ length: 20 }).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      animation: `taskConfetti${i % 8} 1.2s ease-out forwards`,
                      opacity: 0,
                    }}
                  />
                ))}
...
const CONFETTI_COLORS = ['#22c55e', '#fbbf24', '#3b82f6', '#f59e0b', '#a855f7', '#ec4899'];

const confettiKeyframes = Array.from({ length: 8 })
  .map((_, i) => {
    const angle = (i / 8) * 360;
    ...
    return `
      @keyframes taskConfetti${i} {
        ...
        100% { opacity: 0; transform: translate(${x}px, ${y}px) rotate(${rot}deg) scale(0.3); }
      }
    `;
  })
  .join('\n');
...
const overlayKeyframes = `
  ...
  @keyframes taskGlowPulse {
    ...
    30% { box-shadow: inset 0 0 100px rgba(34,197,94,0.5); }
